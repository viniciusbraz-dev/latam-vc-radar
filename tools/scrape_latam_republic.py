"""
scrape_latam_republic.py
Scraper para Latam Republic — notícias LATAM tech.
"""
import json, os, re, requests
from bs4 import BeautifulSoup

HEADERS = {"User-Agent": "Mozilla/5.0 (compatible; LATAMVCRadar/1.0)"}
URL = "https://latamrepublic.com/category/funding/"
OUTPUT = ".tmp/raw_latam_republic.json"
...
def scrape():
    ...
    try:
        ...
        for art in articles[:15]:
            ...
            full = title + ' ' + excerpt
            if not any(k in full.lower() for k in ['raises', 'funding', 'investment', 'round', 'venture']):
                continue
            company = re.sub(r'\s+(raises|secures|closes|gets|lands)\s.*', '', title, flags=re.IGNORECASE).strip()
        ...
        print(f"[latam_republic] erro: {e}")
    os.makedirs(".tmp", exist_ok=True)
    with open(OUTPUT, "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    print(f"[latam_republic] {len(results)} deals encontrados")
    return results
